...
<display:table name="events" id="row" requestURI="${requestURI}"
	...
	<jstl:if test="${currentDate != null}">
		<jstl:choose>
			<jstl:when test="${currentDate > row.moment.time }">
				 <jstl:set var="color" value="background-color:#A4A4A4" />
			</jstl:when>
			<jstl:when test="${(row.moment.time - currentDate > 0) && (row.moment.time - currentDate <= (1000*60*60*24*30)) && (row.seatsNumber - fn:length(row.chorbies)>0)}">
	...
	<display:column title="${pictureH}" style="${color}">
		<img height="65" width="65" src="${row.getPicture()}" />
	</display:column>

	<spring:message code="event.title" var="titleH" />
	<display:column title="${titleH}" property="title" style="${color}"/>

	<spring:message code="event.createMoment" var="createMomentH" />
	<display:column title="${createMomentH}" property="moment" style="${color}"/>

	<spring:message code="event.description" var="descriptionH" />
	<display:column title="${descriptionH}" property="description" style="${color}"/>

	<spring:message code="event.seatsNumber" var="seatsH" />
	<display:column title="${seatsH}" property="seatsNumber" style="${color}"/>

	<spring:message code="event.seatsNumber.available" var="seatsAH" />
	<display:column title="${seatsAH}"
		value="${row.seatsNumber - fn:length(row.chorbies)}" sortable="true" style="${color}"/>

	<!-- REGISTER AND UNREGISTER CHORBIES IN EVENTS -->

	<security:authorize access="hasRole('CHORBI')">

		<display:column style="${color}">

			<jstl:choose>

				<jstl:when
					test="${row.chorbies.contains(principal) && (row.moment > fecha)}">
	...
	<security:authorize access="hasRole('MANAGER')">
		...
		<display:column title="${eEdit}" style="${color}">
			<jstl:choose>
		...
		<display:column title="${eventBroadcast}" style="${color}">
			<jstl:choose>
				<jstl:when test="${principal.events.contains(row) == true}">
